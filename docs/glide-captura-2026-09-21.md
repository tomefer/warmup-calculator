...
La versión normalizada y comprobable por tests está en
[`src/lib/__fixtures__/glide-reference.ts`](../src/lib/__fixtures__/glide-reference.ts),
que es la que consume `glide-parity.test.ts`. Este documento es la materia
prima: sirve para resolver dudas de transcripción sin volver a abrir la app.
